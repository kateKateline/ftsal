...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama)) {
        ...
        $stmt = $conn->prepare("INSERT INTO lapangan (nama, harga_per_jam, status, deskripsi, fasilitas) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sisss", $nama, $harga_per_jam, $status, $deskripsi, $fasilitas);
    ...
    <div class="container">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="number" id="harga_per_jam" name="harga_per_jam" required min="1" value="<?= htmlspecialchars($_POST['harga_per_jam'] ?? '') ?>">
            </div>

            <div class="form-group">
                <label for="status">Status *</label>
                <select id="status" name="status" required>
                    <option value="ready" <?= ($_POST['status'] ?? 'ready') === 'ready' ? 'selected' : '' ?>>Ready</option>
            ...
            <div class="form-group">
                ...
                <textarea id="fasilitas" name="fasilitas"><?= htmlspecialchars($_POST['fasilitas'] ?? '') ?></textarea>
            </div>
